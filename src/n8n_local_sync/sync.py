...
def sync_workflows(client: N8nClient, directory_str: str, force: bool = False, dry_run: bool = False):
    ...
    for wf_id in only_remote:
        ...
        if dry_run:
            typer.echo(f"[DRY-RUN] Would create local file for new remote workflow: {name}")
        else:
    ...
    for wf_id in both:
        ...
        if local_data != remote_data:
            ...
            if not force:
                ...
                if dry_run:
                    typer.secho(f"[DRY-RUN] Would overwrite local file for '{name}' (ID: {wf_id}).", fg=typer.colors.RED)
                else:
    ...
    if dry_run:
        typer.secho(f"\n[DRY-RUN] Sync would complete. {synced_count} workflows would be updated locally.", fg=typer.colors.GREEN)
    else:

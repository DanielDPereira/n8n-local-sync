...
def test_sync_remote_only(temp_dir, mock_client):
    ...
    with patch("n8n_local_sync.sync.get_remote_workflows", return_value=remote_workflows):
        with patch("n8n_local_sync.sync.get_local_workflows", return_value={}):
...
def test_sync_local_modified(temp_dir, mock_client):
    ...
    with patch("n8n_local_sync.sync.get_remote_workflows", return_value={"1": remote_data}):
        with patch("n8n_local_sync.sync.get_local_workflows", return_value={"1": local_data}):
